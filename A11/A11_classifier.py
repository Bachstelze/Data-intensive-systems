...
RESULTS_DIR = Path(os.getcwd()) / 'A11_results'
...
def build_dense(input_dim, hidden_units=(128, 64),
                ...
                l2_reg=1e-4, n_classes=3, name='Dense'):
...
def build_conv1d(input_dim, window_size=5,
                 filters=(64, 128), kernel_size=3,
                 dense_units=(64,), dropout_rate=0.2,
                 n_classes=3, name='Conv1D'):
...
def build_lstm(input_dim, window_size=5,
               lstm_units=(64, 32), dense_units=(32,),
               dropout_rate=0.2, n_classes=3, name='LSTM'):
    inputs = keras.Input(shape=(window_size, input_dim), name='input')
    x = inputs
...
def build_gru(input_dim, window_size=5,
              gru_units=(64, 32), dense_units=(32,),
              dropout_rate=0.2, n_classes=3, name='GRU'):
    inputs = keras.Input(shape=(window_size, input_dim), name='input')
    x = inputs
...
def run_10fold_cv(X_trainval, y_trainval, X_test, y_test,
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(
        ...
        if is_seq:
            X_tr, y_tr = create_sequences(X_tr, y_tr, window_size=5)
            X_val, y_val = create_sequences(X_val, y_val, window_size=5)
        ...
        p_start = precision_score(y_val, y_pred, labels=[1],
                                  average='macro', zero_division=0)
        r_start = recall_score(y_val, y_pred, labels=[1],
                               average='macro', zero_division=0)
        f1_start = f1_score(y_val, y_pred, labels=[1],
                            average='macro', zero_division=0)
        p_stop  = precision_score(y_val, y_pred, labels=[2],
                                  average='macro', zero_division=0)
        r_stop  = recall_score(y_val, y_pred, labels=[2],
                               average='macro', zero_division=0)
        f1_stop = f1_score(y_val, y_pred, labels=[2],
                           average='macro', zero_division=0)
        avg_f1  = (f1_start + f1_stop) / 2

        print(f"START F1={f1_start:.3f}  STOP F1={f1_stop:.3f}  "
              f"epochs={len(history.history['loss'])}")

        fold_metrics.append({
            'fold': fold_idx + 1,
            'p_start': p_start, 'r_start': r_start, 'f1_start': f1_start,
            'p_stop':  p_stop,  'r_stop':  r_stop,  'f1_stop':  f1_stop,
            'avg_f1':  avg_f1,
    ...
    print(f"  START P:{avg['p_start']:.3f} R:{avg['r_start']:.3f} "
          f"F1:{avg['f1_start']:.3f}")
    print(f"  STOP  P:{avg['p_stop']:.3f} R:{avg['r_stop']:.3f} "
          f"F1:{avg['f1_stop']:.3f}")
    ...
    if is_seq:
        ...
        X_test_final, y_test_final = create_sequences(X_test, y_test, window_size=5)
    ...
    return {
        ...
        'optimizer':    optimizer,
        'batch_size':   batch_size,
        'avg_p_start':  avg['p_start'],
        'avg_r_start':  avg['r_start'],
        'avg_f1_start': avg['f1_start'],
        'avg_p_stop':   avg['p_stop'],
        'avg_r_stop':   avg['r_stop'],
        'avg_f1_stop':  avg['f1_stop'],
        'avg_epochs':   avg['epochs'],
        'test_f1_start': f1_score(y_test_final, y_pred_test, labels=[1],
                                  average='macro', zero_division=0),
        'test_f1_stop':  f1_score(y_test_final, y_pred_test, labels=[2],
                                  average='macro', zero_division=0),
    }
...
def assess_frame_offset(y_true, y_pred, problem_name):
    true_starts = np.where(y_true == 1)[0]
    pred_starts = np.where(y_pred == 1)[0]
    true_stops  = np.where(y_true == 2)[0]
    pred_stops  = np.where(y_pred == 2)[0]

    print(f"\n  Frame Offset Analysis ({problem_name}):")

    for label, true_pos, pred_pos in [
        ('START', true_starts, pred_starts),
        ('STOP',  true_stops,  pred_stops)
    ]:
        if len(pred_pos) > 0 and len(true_pos) > 0:
            offsets = [
                abs(tp - pred_pos[np.argmin(np.abs(pred_pos - tp))])
                for tp in true_pos
            ]
            avg = np.mean(offsets)
            print(f"    {label}: avg offset = {avg:.1f} frames "
                  f"= {avg/30:.3f} sec at 30fps "
                  f"(predicted {len(pred_pos)} frames)")
        else:
            missed = len(true_pos)
            print(f"    {label}: no predictions made "
                  f"(missed all {missed} true {label.lower()} frames)")
...
def make_architectures(input_dim):
    return {
        'Dense_relu':   lambda d: build_dense(
            d, hidden_units=(128, 64), activation='relu', dropout_rate=0.2),
        'Conv1D':       lambda d: build_conv1d(
            d, window_size=5, filters=(64, 128), dropout_rate=0.2),
        'LSTM':         lambda d: build_lstm(
            d, window_size=5, lstm_units=(64, 32), dropout_rate=0.2),
        'GRU':          lambda d: build_gru(
            d, window_size=5, gru_units=(64, 32), dropout_rate=0.2),
    }
...
results_df = results_df.sort_values('avg_f1_start', ascending=False)

print(f"\n{'='*70}")
print("FINAL RESULTS — All experiments sorted by START F1")
print(f"{'='*70}")
print(results_df[[
    'problem', 'architecture', 'optimizer', 'batch_size',
    'avg_f1_start', 'avg_f1_stop',
    'test_f1_start', 'test_f1_stop', 'avg_epochs'
...
print(f"{'CV START F1':<28} {best_A['avg_f1_start']:>12.3f} {best_B['avg_f1_start']:>12.3f}")
print(f"{'CV STOP F1':<28} {best_A['avg_f1_stop']:>12.3f} {best_B['avg_f1_stop']:>12.3f}")
print(f"{'Test START F1':<28} {best_A['test_f1_start']:>12.3f} {best_B['test_f1_start']:>12.3f}")
print(f"{'Test STOP F1':<28} {best_A['test_f1_stop']:>12.3f} {best_B['test_f1_stop']:>12.3f}")

diff = best_A['avg_f1_start'] - best_B['avg_f1_start']
if diff > 0.02:
